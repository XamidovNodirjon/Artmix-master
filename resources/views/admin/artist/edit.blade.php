...
@section('content')
    <div class="container">
        ...
        <form action="{{ route('update-artist', $artists->id) }}" method="POST" enctype="multipart/form-data"
              id="artist-form">
            @csrf
            @method('PUT')
            ...
            <div class="mb-3">
                ...
                <textarea name="description" class="form-control"
                          rows="4">{{ old('description', $artists->description) }}</textarea>
            </div>
            <div class="mb-3">
                        ...
                        <div class="card mb-3 work-item" data-work-id="{{ $work->id }}">
                            <div class="card-header d-flex justify-content-between align-items-center">
                                ...
                                <button type="button" class="btn btn-sm btn-danger remove-work"
                                        data-work-id="{{ $work->id }}">O'chirish
                                </button>
                            </div>
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="mb-3">
                                            ...
                                            <input type="text" name="works[{{ $work->id }}][size]" class="form-control"
                                                   value="{{ old("works.$work->id.size", $work->size) }}" required>
                                        </div>
                                        <div class="mb-3">
                                            <label class="form-label">Materiallari</label>
                                            <input type="text" name="works[{{ $work->id }}][materials]"
                                                   class="form-control"
                                                   value="{{ old("works.$work->id.materials", $work->materials) }}"
                                                   required>
                                        </div>
                                        <div class="mb-3">
                                            <label class="form-label">Rasm nomi</label>
                                            <input type="text" name="works[{{ $work->id }}][image_name]"
                                                   class="form-control"
                                                   value="{{ old("works.$work->id.image_name", $work->image_name) }}"
                                                   required>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-md-6">
                                            ...
                                            @if($work->images && Storage::disk('public')->exists($work->images))
                                                <a href="{{ asset('storage/' . $work->images) }}" target="_blank">
                                                    <img src="{{ asset('storage/' . $work->images) }}"
                                                         class="img-thumbnail preview-thumbnail" alt="Asar rasmi">
                                                </a>
                                                <input type="hidden" name="works[{{ $work->id }}][existing_image]"
                                                       value="{{ $work->images }}">
                                            @else
                                        ...
                                        <div class="mt-2">
                                            ...
                                            <input type="file" name="works[{{ $work->id }}][new_image]"
                                                   class="form-control" id="images" accept="image/*">
            ...
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="file" name="works[new-__INDEX__][new_image]" class="form-control" id="images"
                               accept="image/*">
                        <div id="file-error" style="color: red; margin-top: 6px;"></div>
                    </div>
                </div>
